...
#load hra hh size by tenure
hra_hhsize_by_tenure <- read.csv(
    file = "./data/hra_hhsize_by_tenure.csv",
    colClasses = c("GEOID" = "character")
)
while (!exists("hra_hhsize_by_tenure")) {
    Sys.sleep(1)
}
    ...
    sp_reactive <- reactive({
        ...
        if (geo_reactive()) {
            ...
            selected_df <- hra_proj %>%
                ...
                selected_df <- hra_hhsize_by_tenure %>%
                    filter(Year %in% year_reactive())
...
#ARA: These filter by user-inputted variable specific demographics
        ...
        #ARA FIX: for HRA, might need to group by HRA 
            ...
            selected_df <- selected_df %>%
                filter(tenure %in% tenure_reactive())%>%
                group_by(GEOID) %>%
                summarize(value = sum(value),
                          moe = sum(moe))
    ...
    legend_title_reactive <- reactive({
        if (input$measure_type == "Count") {
            if(var_reactive() == "Population"){
                ...
                "Individuals with Education Status"
            }
            else if(var_reactive() == "Rent Burden"){
                ...
                "Household Population According to Tenure"
            }
            
            else if(var_reactive() == "Methods of Transportation to Work - Median Age"){
                "Population Acccoridng to Method of Transportation"
            }
    ...
    popup_text_reactive <- reactive({
        paste0(
            ...
            if(input$measure_type == "Count"){
                    ...
                    "Number of Individuals: <strong>%d</strong><br/>"
    ...
    observe({
        ...
        if (1>2) {
            ...
            legend_values <- quantile(sp@data$value, 
                                      type = 5, names = FALSE, na.rm = TRUE)
