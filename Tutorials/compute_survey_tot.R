compute_survey_tot <- function(dataframe){
  ...
  multigen <- multigen %>% right_join(multigen)
  ...
  names(multigen)[which(names(multigen)=="se")]<-"se.average_hh_size"
  ...
  names(multigen3)[which(names(multigen3)=="se")]<-"se.prevalence.2_person_hh"
  
  # merge results with empty objects #
  print("Merging results.")
  hh_type<-hh_type3%>%right_join(hh_type)
  hh_tenure<-hh_tenure3%>%right_join(hh_tenure)
  multigen<-multigen3%>%right_join(multigen)
