...
hhDF <- ipums_recode_acs5(ipums) %>%
  ...
  dplyr::select(YEAR, STRATA, CLUSTER, hh_id, HHWT, hh_type, tenure, hh_size) %>%
...
my.svydesign <- svydesign(id= ~hh_id,
                              strata=~STRATA,
...
###############################################################
test <-svyby(~hh_size,
             ~hh_type+tenure,my.svydesign,svytable,na.rm=T)

tmp <- svytable(~hh_size + hh_type + tenure, my.svydesign)
