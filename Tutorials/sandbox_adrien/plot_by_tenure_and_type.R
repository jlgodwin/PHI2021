...
p1 <- plotDF %>%  
  ...
  mutate(hh_size = ifelse(hh_size %in% 4:7, "4+", as.character(hh_size)),
         hh_size = factor(hh_size)
  ) %>%
  group_by(
    year, tenure, hh_size, hh_type
  ) %>%
  summarize(
    value = sum(value),
    hh_pop = sum(hh_pop)
...
ggsave(p1, filename = paste0(code_dir, "Report_plots/hh_by_size_tenure_type(4+).png"), width = 11, height = 6)
...
diffplot <- plotDF %>%
  ...
  summarize(
    value = sum(value),
    value2000 = sum(value2000),
    hh_pop = sum(hh_pop),
    diff2000 = sum(diff2000)
  )
...
p2 <- diffplot %>%  
  ...
  geom_line(data = diffplot %>% filter(hh_type != "Family" | hh_size != 1) %>%
              filter(tenure != "ALL" & hh_type != "ALL"),# %>% filter(diff2000 > 5000),
            aes(color = hh_size),
            size = 1.2) +
  geom_point(data = diffplot %>% filter(hh_type != "Family" | hh_size != 1) %>%
               filter(tenure != "ALL" & hh_type != "ALL"), #%>% filter(diff2000 > 5000),
             aes(color = hh_size, size = 1.5)) +
  # geom_ribbon(aes(fill = hh_size), alpha = .4) +
  theme_classic() +
...
ggsave(p2, filename = paste0(code_dir, "Report_plots/diff_since_2000_hh_by_size_tenure_type(4+).png"), width = 11, height = 6)
...
p3 <- popDF %>%  
  ...
  geom_line(data = popDF %>%  
              filter(tenure != "ALL" & hh_type != "ALL")%>%
              filter(hh_type != "Family" | hh_size != 1),#%>%filter(hh_pop > 5000) ,
            aes(color = hh_size),
            size = 1.2) +
  geom_point(aes(color = hh_size, size = 1.5)) +
  # geom_ribbon(aes(fill = hh_size), alpha = .4) +
...
ggsave(p3, filename = paste0(code_dir, "Report_plots/hh_pop_by_size_tenure_type(4+).png"), width = 11, height = 6)
...
ggsave(p4, filename = paste0(code_dir, "Report_plots/diff_since_2000_hh_pop_by_size_tenure_type(4+).png"), width = 11, height = 6)
