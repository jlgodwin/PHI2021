...
acs5DF <- bind_rows(lapply(years, function(x){
  ...
  left_join(
    bind_rows(lapply(c(2014,2019), function(x){
      get_acs(
        "tract",
        table = c("B25027"
        ),
...
saveRDS(totDF, file = paste0(out_dir, "census_tract_survey_data.RDS"))
saveRDS(hhsizeDF, file = paste0(out_dir, "census_tract_hh_by_size.RDS"))
